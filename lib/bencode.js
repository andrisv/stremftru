...
Bencode.encode = function(input) {
  if (input instanceof Buffer) {
    return Buffer.concat([ Buffer.from(input.length.toString() + ':'), input ]);
    ...
    return Buffer.concat([ Buffer.from('d'), Buffer.concat(bufs), Buffer.from('e') ]);

  } else if (typeof(input) === 'string') {
    return Buffer.from(input.length.toString() + ':' + input);

  } else if (typeof(input) === 'number') {
    return Buffer.from('i' + input + 'e');
